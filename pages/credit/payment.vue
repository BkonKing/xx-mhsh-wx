<template>
  <view>
    ...
    <view class="tf-main-container">
      <tf-user-info :avatar="info.avatar" :name="info.realname" :time="info.mobile"></tf-user-info>
    ...
    <tf-dialog
      ...
      @closed="paypassword = ''"
    >
      <tf-password-input v-model="paypassword" ref="passwordInput" />
    </tf-dialog>
  </view>
</template>
...
export default {
  ...
  onLoad(options) {
    const { type, value, avatar, realname, mobile, credits, remarks } = options
    this.type = type
    this.codeId = value
        ...
        uni.showModal({
          ...
          success: (res) => {
            if (res.confirm) {
              ...
              this.$nextTick(() => {
                this.$router.push('/pages/personage/information/payment-code?status=0')
              })
            } else if (res.cancel) {
              this.$refs.passwordInput.focus()
            }
